...
dirs = [os.path.join(source_dir,d) for d in ld if "runs1_new" in d and os.path.isdir(os.path.join(source_dir,d))]
dirs.sort()
...
for c, cond in enumerate(dirs):
    ...
    for f, f_par in enumerate(np.unique(parameters[var_color])):
        ...
        plt.plot(t, (dist[ind,:].T-dist[ind,0])/np.array(parameters[ind]["anchor_stiffness"]))
        plt.xlabel("Time (s)")
