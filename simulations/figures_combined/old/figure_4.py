...
def final_length(Dm,rho_m,v0,Lc,L_max,sugg=1.5):
    ...
    for i in range(Lc.shape[0]):
        res = fsolve(eq2solve,Lc[i]*sugg,args=(Dm,rho_m,v0,Lc[i]))[0]
        if res>L_max:
            out.append(L_max)
        else:
            out.append(res)
...
def speed_entropic(L,c,D,visc,L_mt,a,do_occup=True):
    ent = entropic(c,L,a)
    d = drag(D,c,L,a,visc,L_mt,do_occup)
    return ent/d, d, ent
...
for i,d in enumerate(dirs):
    ...
    if d==source2:
        ...
        max_speed = speed2(Dc, Dm, v0, 6., c_num*0.008/L, ave_rhom, L)/2.
...
x = np.linspace(0,0.8)
plt.plot(x,x,color="black",label="$v_f/v_0=v_g/v_0$")
plt.legend(frameon=False)
plt.ylabel("$v_f/v_0$")
plt.xlabel("$v_g/v_0$")
plt.ylim([0,0.8])
...
for f,f_par in enumerate(np.unique(occup)):
    ind = occup==f_par
    v,dra,ent = speed_entropic(L,f_par*L/a,D,L_mt,visc,a)
