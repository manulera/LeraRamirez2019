...
for c, cond in enumerate(dirs):
    ...
    for f, f_par in enumerate(np.unique(parameters[var_color])):
        ...
        sc = plt.scatter(predictions["bound_cl"][ind], measurements["speed"][ind],label=var_color + " " +str(f_par))
        sort_plot(predictions["bound_cl"][ind], predictions["speed"][ind],plt.plot,color=sc.get_facecolors()[0])
