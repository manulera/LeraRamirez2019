...
for c, cond in enumerate(dirs[-1:]):
    ...
    for f, f_par in enumerate(np.unique(parameters[var_color])):
        ...
        if cond!="../data_summary/runs4":
            sc = plt.scatter(measurements["bound_cls"][ind], measurements["speed"][ind],label=var_color + " " +str(f_par))
            sort_plot(measurements["bound_cls"][ind], predictions["speed"][ind],plt.plot,color=sc.get_facecolors()[0])
        else:
