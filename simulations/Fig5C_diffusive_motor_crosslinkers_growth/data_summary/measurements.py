...
def measure(path):
    ...
    meas = dict()
    meas_names = ["speed","bound_mts","bound_cls","final_length","rho_m","force_cls","force_mts"]

    for i, name in enumerate(meas_names):
        meas.update({name: i})
    dist = manual_parsing(os.path.join(path, "results.txt"), " ", float)
    overlap = manual_parsing(os.path.join(path, "results_overlap.txt"), " ", float)
    ...
    measurements[:,meas["speed"]] = calc_speed(t[40:],dist[:,40:])
    measurements[:,meas["bound_mts"]] = np.mean(couples[:,40::2],axis=1)
    measurements[:, meas["bound_cls"]] = np.mean(couples[:,41::2],axis=1)
    measurements[:, meas["rho_m"]] = np.mean(couples[:, 40::2]/overlap[:, 20:], axis=1)*0.008
    measurements[:, meas["final_length"]] = np.mean(overlap[:, 20:], axis=1)

    # measurements[:, meas["force_mts"]] = forces[:,0]
    # measurements[:, meas["force_cls"]] = forces[:,1]
