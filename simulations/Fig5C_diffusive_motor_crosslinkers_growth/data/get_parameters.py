...
def get_vals(pile,extract):
    ...
    for i in range(0, len(extract), 3):
        ...
        if len(com)==3:
            ...
            obj = read_config.get_command(pile, com[:3])
            obj = obj[com[-1]]
        if len(what)==1:
...
extract =[
    ...
    ['set', 'hand', 'motor_hand_diff'],  ["diffusion"],          "mt_diff",
    ['set', 'hand', 'cross_hand'],  ["diffusion"],          "cl_diff",
    ['set', 'fiber', 'mt'],  ["growing_speed"],          "growth_speed",
]
...
extract = [
    ...
    ['run', 'simul', '*',2], ["nb_steps"], "nb_steps",
    ['run', 'simul', '*',2], ["nb_frames"], "nb_frames",
]
par_names+=extract[2::3]
out_list += get_vals(pile,extract)
