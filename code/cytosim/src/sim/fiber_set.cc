...
/**
 The initialization options depend on the type of fiber: Fiber, DynamicFiber, ClassicFiber, Tubule.
 ...
 In this case, the Single/Couple are anchored at random position distributed uniformly along the Fiber.

 @code
 new fiber microtubule
 {
 attach0 = [NUMBER] NAME, DISTANCE, REFERENCE [, MODIFIER]
 }
 @endcode
 
 If `DISTANCE` is specified, the Single/Couple will be attached at a distance from the `REFERENCE`,
 which be `minus_end`, `plus_end` or `center` (default = `minus_end`).
 Moreover, a `MODIFIER` can be given: `uniform`, `exponential` or `regular` (by default: `uniform`).
 With `uniform` Single/Couple are attached uniformly at distance `[0, DISTANCE]` from the `REFERENCE`.
 With `regular`, they are distributed regularly.
 
 You can also specify multiple attachement instructions with `attach1`, `attach2`, etc.
 For example, to attach one `protein` at each end of the filaments, one can use:
 @code
 new fiber minifilament
 {
    attach0 = protein, 0.0, minus_end
    attach1 = protein, 0.0, plus_end
 }
 @endcode
...
{
    Property * p = simul.properties.find_or_die("fiber", name);
    Fiber * fib = static_cast<FiberProp*>(p)->newFiber(opt);
    ...
    int inp = 0;
    std::string spe, var = "attach0";
    
#ifdef BACKWARD_COMPATIBILITY
    if ( !opt.has_key(var) && opt.has_key("attach") )
        var = "attach";
#endif
    ...
    {
        unsigned cnt = 1;
        Tokenizer::split_integer(spe, cnt);
        
        real len = fib->length();
        opt.set(len, var, 1);
        
        FiberEnd ref = MINUS_END;
        opt.set(ref, var, KeyList<FiberEnd>("plus_end", PLUS_END, "minus_end", MINUS_END, "center", CENTER), 2);
        int mod = 0;
        opt.set(mod, var, KeyList<int>("uniform", 1, "exponential", 2, "regular", 3), 3);
        
        // search for Single and Couple:
        SingleProp * sip = simul.findProperty<SingleProp*>("single", spe);
        CoupleProp * cop = simul.findProperty<CoupleProp*>("couple", spe);
        ...
        for ( unsigned n = 0; n < cnt; ++n )
        {
            ...
            real abs = fib->abscissaFrom(a, ref);
 ...
 */
Object * FiberSet::newObjectT(const Tag tag, unsigned idx)
{
    Fiber * obj = 0;
...
#endif
    {
        ...
        if ( p == 0 )
            throw InvalidIO("no fiber class defined with id "+sMath::repr(idx));
        obj = p->newFiber();
...
void FiberSet::step()
{
    ...
    for ( PropertyList::iterator pi = plist.begin(); pi < plist.end(); ++pi )
        static_cast<FiberProp*>(*pi)->total_length = 0;
    ...
    for ( PropertyList::iterator pi = plist.begin(); pi < plist.end(); ++pi )
    {
        FiberProp * p = static_cast<FiberProp*>(*pi);
        ...
        if ( p->free_polymer < 0 )
        {
            MSG.warning("The free monomer concentration would be negative !!!\n");
...
/**
 ...
 and report the corresponding abscissa in array 'res'.
 */
void FiberSet::allIntersections(Array<FiberBinder>& res, const real max_distance) const
{
    const real mds = max_distance * max_distance;
    res.clear();

    for ( Fiber * fib1 = first(); fib1; fib1 = fib1->next() )
    {
        for ( unsigned seg1 = 0; seg1 < fib1->nbSegments(); ++seg1 )
        {
            FiberLocus & loc1 = fib1->locus(seg1);
            for ( Fiber * fib2 = fib1->next(); fib2; fib2 = fib2->next() )
            {
                for ( unsigned seg2 = 0; seg2 < fib2->nbSegments(); ++seg2 )
                {
                    FiberLocus & loc2 = fib2->locus(seg2);
                    real abs1, abs2, dis;
                    if ( loc1.shortestDistance(loc2, abs1, abs2, dis) )
                    {
                        if ( dis < mds )
                        {
                            res.push_back(FiberBinder(fib1, abs1+fib1->abscissaPoint(seg1)));
                            res.push_back(FiberBinder(fib2, abs2+fib2->abscissaPoint(seg2)));
                        }
                    }
                }
            }
 ...
 */
void FiberSet::newFiberSitesP(Array<FiberBinder>& res, const real spread) const
{
    assert_true( spread > 0 );
    ...
    while ( fib )
    {
        ...
        while ( abs < len )
        {
            res.push_back(FiberBinder(fib, fib->abscissaP()-abs));
            abs += spread * RNG.exponential();
        }
        abs -= len;
        fib = fib->next();
 ...
 */
void FiberSet::newFiberSitesM(Array<FiberBinder>& res, const real spread) const
{
    assert_true( spread > 0 );
    ...
    while ( fib )
    {
        ...
        while ( abs < a )
        {
            res.push_back(FiberBinder(fib, fib->abscissaM()+abs));
            abs += spread * RNG.exponential();
        }
        abs -= a;
        fib = fib->next();
...
void FiberSet::infoBirthtime(ObjectList const& objs, unsigned& cnt, real& avg, real& dev, real& mn, real& mx)
{
    cnt = 0;
    avg = 0;
    dev = 0;
...
void FiberSet::infoSegments(ObjectList const& objs,
                            unsigned& cnt, unsigned& joints, unsigned& kinks, real& mn, real& mx)
{
    cnt = 0;
    joints = 0;
    kinks = 0;
                           ...
                           real res[9])
{
    ...
    int info = 0;
    lapack_xsyevx('V','I','L', DIM, M, 3, 0, 0, 2, DIM, REAL_EPSILON,
...
/**
 Return the principal component directions of the cloud of model points.
                             ...
                             real& sum, real avg[3], real mom[9], real res[9])
{
    ...
    //VecPrint::matPrint(std::clog, 3, 3, vec);
...
void FiberSet::infoTension(unsigned& cnt, real& ten, Vector const& n, real a) const
{
    ...
    for ( Fiber const* fib=first(); fib; fib=fib->next() )
    {
        for ( unsigned s = 0; s < fib->nbSegments(); ++s )
        {
            ...
            if ( 0 <= abs  &&  abs < 1 )
            {
                ten += fabs( dir * fib->dirPoint(s) ) * fib->tension(s);
                ++cnt;
            }
        }
    }
...
void FiberSet::infoLattice(real& len, real& sum, real& mn, real& mx) const
{
    len = 0;
    sum = 0;
    mn = INFINITY;
    ...
    for ( Fiber const* fib=first(); fib; fib=fib->next() )
    {
        ...
        {
            len += fib->length();
            const real unit = lat->unit();
            const int sup = lat->index(fib->abscissaP());
            for ( int ix = lat->index(fib->abscissaM()); ix <= sup; ++ix )
            {
                sum += lat->value(ix);
                real x = lat->value(ix) / unit; // density
                if ( x < mn ) mn = x;
                if ( x > mx ) mx = x;
            }
        }
