...
void WristLong::setInteractions(Meca & meca) const
{
    ...
    meca.interSideLink2D(pt, PointExact(mBase, mPoint), mArm, prop->stiffness);
    
#elif ( DIM == 3 )
    
    mArm = calcArm(pt, posFoot(), prop->length);
    meca.interSideLinkS(pt, PointExact(mBase, mPoint), mArm, prop->length, prop->stiffness);
