...
void PointGrid::checkPP(Meca& meca, PointGridParam const& pam, FatPoint const& aa, FatPoint const& bb) const
{
    ...
    if ( d < ran*ran )
    {
        ...
        if ( d > len*len )
            meca.interLongLink(aa.pe, bb.pe, len, pam.stiff_pull);
        else
    ...
    if ( 0 <= a )
    {
        if ( a <= bb.fl.len() )
        {
            if ( d < ran*ran )
            {
                ...
                if ( d > len*len )
                    meca.interSideSlidingLink(bi, aa.pe, len, pam.stiff_pull);
                else
        ...
        else
        {
            ...
            if ( vab * bb.fl.fiber()->diffPoints(bb.fl.point()-1) >= 0 )
            {
                ...
                if ( d < ran*ran )
                {
                    const real len = aa.radius + bb.radius;
                    if ( d > len*len )
                        meca.interLongLink(aa.pe, bb.fl.exact1(), len, pam.stiff_pull);
                    else
