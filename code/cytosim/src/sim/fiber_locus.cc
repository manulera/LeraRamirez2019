 ...
 */
real FiberLocus::projectPoint0(Vector const& w, real& dis) const
{
    assert_true( mFib );
 ...
 */
real FiberLocus::projectPoint(Vector const& w, real& dis) const
{
    assert_true( mFib );
 ...
 */
real FiberLocus::projectPointF(const real w[], real& dis) const
{
    assert_true( mFib );
...
int FiberLocus::shortestDistance(FiberLocus const& that, real& abs1, real& abs2, real& dis) const
{
    Vector d1  = diff();
    Vector d2  = that.diff();
...
std::ostream& operator << (std::ostream& os, FiberLocus const& p)
{
    if ( p.fiber() )
        os << "(" << p.fiber()->reference() << " seg " << p.point() << ":" << p.point()+1 << ")";
    else
        os << "(null)";
