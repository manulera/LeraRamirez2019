...
public:
    ...
    /**
     ...
     - 1: attach Couple along the length of fibers
       ...
       count of the number of free motors, and directly attach these Couples by 
       ...
       In this mode, the diffusion constant is not relevant.

     - 2: attach Couple only near growing PLUS_ENDS
       Similar to mode 1, but Hand1 of Couple is directly attached at random 
       positions on the plus tips of growing fibers. The number of attachments
       is proportional to the new polymer mass, which is time_step * growing_speed
     .
     
     Enabling `fast_diffusion` can make the simulation a bit faster when there
     are a lot of Couples, but another reason to use this is that it makes the
     model simpler, since the fraction of bound/free can then be calculated
    ...
    /**
     ...
     it has docked on an existing Fiber.
     */
    ...
    /**
     This can limit the binding to only certain configurations:
     - `off` (default): no restriction
     - `parallel`: can only link two parallel fibers if their angle is +/- 60 degrees
     - `antiparallel`: can only link anti-parallel fibers if their angle is +/- 60 degrees
     */
    ...
    // ADDED BY MANU:
    /// Marks that this couple step is done with Gillespie for separation of timescales
    bool is_gillespie;
...
public:
    ...
    virtual Couple * newCouple(Glossary * = 0) const;
