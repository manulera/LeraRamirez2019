...
{
    KeyList<int> conditions("flux", 0, "edge", 7,
    ...
    glos.set(boundary_condition, "boundary_condition", conditions);
    glos.set(boundary_value,     "boundary_value");
    glos.set(boundary_condition, "boundary", conditions);
    glos.set(boundary_value,     "boundary", 1);
    glos.set(decay_rate,         "decay_rate");
    
    if ( glos.set(bind_fibers, "bind_fibers") )
        MSG.warning("field::bind_fibers is disabled, use fiber:lattice_binding_rate\n");
...
void FieldProp::complete(Simul const* sim)
{
    ...
    if ( sim->prop->strict  &&  !confine_space_ptr )
...
void FieldProp::write_values(std::ostream & os) const
{
    ...
    write_value(os, "bind_fibers",    bind_fibers);
