...
int DynamicFiber::stepPlusEnd()
{
    ...
    else
    {
        ...
        while ( nextGrowthP < 0 || nextShrinkP < 0 || nextHydrolP < 0 )
        {
            ...
            int ii = sMath::arg_min(nextGrowthP/growth_rate, nextShrinkP/detach_rate, nextHydrolP/hydrol_rate);
...
void DynamicFiber::step()
{
    ...
    int addM = stepMinusEnd();
    
    mGrowthM = addM * prop->unit_length;

    if ( addM )
    {
        if ( length() + mGrowthM < prop->min_length )
        {
            // do something if the fiber is too short:
            if ( !prop->persistent )
            {
...
void DynamicFiber::read(Inputter & in, Simul& sim, Tag tag)
{
