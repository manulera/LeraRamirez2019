...
int buildMenuFiber()
{
    ...
    if ( FDisp )
    {
        ...
        assert_true( dproperties.contains(FD) );
...
{
    PropertyList dps = dproperties.find_all("bead:display", "solid:display");
    if ( dps.size() )
    {
        Property * pp = dps[0];
...
void processMenuExport(int item)
{
    ...
    {
        case 0:   return;
        case 1:   saveImage("image", PP.image_index++); return;
        case 2:   saveMagnifiedImage(3, "image", PP.image_index++, 3); return;
        case 3:   saveMagnifiedImage(6, "image", PP.image_index++, 3); return;
        case 4:   PP.save_images = 1; startPlayback();  return;
        
        case 5:   saveMagnifiedImage(2, "poster", PP.poster_index++);   return;
        case 6:   saveMagnifiedImage(3, "poster", PP.poster_index++);   return;
        case 7:   saveMagnifiedImage(4, "poster", PP.poster_index++);   return;
        case 8:   saveMagnifiedImage(6, "poster", PP.poster_index++);   return;
        ...
        case 23:  simThread.exportObjects(false);             return;
        case 24:  simThread.exportObjects(true);              return;
...
int buildMenuExport()
{
    ...
    glutAddMenuEntry("Save Nice Image",           2);
    glutAddMenuEntry("Save 2x Nice Image",        3);
    glutAddMenuEntry("Play & Save Images (Y)",    4);
    glutAddMenuEntry("Save 2x Magnified Poster",  5);
    glutAddMenuEntry("Save 3x Magnified Poster",  6);
    glutAddMenuEntry("Save 4x Magnified Poster",  7);
    glutAddMenuEntry("Save 6x Magnified Poster",  8);
...
void processMenu(int item)
{
    switch( item )
    {
        ...
        case 8: glApp::createWindow(); break;
