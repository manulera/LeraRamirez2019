...
void FileWrapper::put_line(const char * str, char sep)
{
    ...
    // check if terminating character appears in the string:
    char * m = (char*)memchr(str, sep, s);
    
    if ( m )
        fwrite(str, 1, 1+m-str, mFile);
    else
...
void FileWrapper::put_line(const std::string& str, char sep)
{
    put_line(str.c_str(), sep);
}
