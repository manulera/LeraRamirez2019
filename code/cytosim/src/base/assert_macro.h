...
/**
 Assertions are used at various places to check the validity of the arguments.
 Defining NDEBUG disables:
 - the standard assert() macro and
 - the custom assert macros defined below,
...
*/
  ...
  #define assert_small(expression)\
        { real e = expression; real m = 128*REAL_EPSILON;\
        if ( e > m || e < -m ) {\
            fprintf(stderr, "- - - - - - - - - - - - - - - - - - - - - - - - - -\n");\
