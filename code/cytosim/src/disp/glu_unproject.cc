...
void multMatrixVecd(const FLOAT matrix[16], const FLOAT in[4], FLOAT out[4])
{
    for (int i=0; i<4; i++)
...
// Invert 3x3 matrix.
int invertMatrix3(const FLOAT m[9], FLOAT inv[9])
{
 ...
 */
int invertMatrix4(const FLOAT m[16], FLOAT inv[16])
{
...
void multMatricesd(const FLOAT a[16], const FLOAT b[16], FLOAT r[16])
{
    int i, j;
                   ...
                   FLOAT *objx, FLOAT *objy, FLOAT *objz)
{
    ...
    multMatricesd(modelMatrix, projMatrix, mat);
    
    if (!invertMatrix4(mat, inv))
        return GL_FALSE;
