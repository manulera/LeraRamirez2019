...
public:
    ...
    /// returns a vector perpendicular to *this, close to `d`, of norm = `n`
    const Vector1 orthogonal(Vector1, const real)
    {
        ABORT_NOW("Vector::orthogonal() is meaningless in 1D");
        return Vector1(0, 0);
    }
    ...
    friend real cross(Vector1 const& a, Vector1 const& b)
    {
        return 0;
    }

    /// cross product of a vector with a Z-Vector
    friend const Vector1 cross(Vector1 const& a, const real b)
    {
        return Vector1(0, 0);
    }

    /// cross product of a Z-vector with a Vector
    friend const Vector1 cross(const real a, Vector1 const& b)
    {
        return Vector1(0, 0);
    }
