...
//#   define MATRIX2_USES_INTEL_SSE3
#endif
...
#ifdef MATRIX2_OPTIMIZED_MULTIPLY
    nmax_      = 0;
    ija_       = 0;
    sa_        = 0;
...
void MatrixSparseSymmetric2::allocate(unsigned alc)
{
    if ( alc > allocated_ )
    {
...
void MatrixSparseSymmetric2::allocateColumn(const index_type jj, unsigned alc)
{
...
*/
real& MatrixSparseSymmetric2::operator()(index_type ii, index_type jj)
{
...
real* MatrixSparseSymmetric2::addr(index_type ii, index_type jj) const
{
    // swap to get ii <= jj (address lower triangle)
    if ( ii < jj )
        std::swap(ii, jj);
...
void MatrixSparseSymmetric2::makeZero()
{
...
unsigned MatrixSparseSymmetric2::nbElements() const
{
...
#ifdef MATRIX2_USES_INTEL_SSE3
    msg << "mSS2i (" << nbElements() << ")";
#else
    msg << "mSS2 (" << nbElements() << ")";
#endif
    return msg.str();
...
void MatrixSparseSymmetric2::printColumn(std::ostream & os, const index_type jj)
{
...
#ifndef MATRIX2_OPTIMIZED_MULTIPLY

void MatrixSparseSymmetric2::prepareForMultiply()
{
}
...
void MatrixSparseSymmetric2::vecMulAddIso2D(const real* X, real* Y) const
{
    for ( index_type jj = col_next_[0]; jj < size_; jj = col_next_[jj+1] )
    {
        ...
        for ( index_type kk = ija_[jj]; kk < end; ++kk )
        {
            vec2 tt;
            aa = SSE(load1)(sa_+kk);
            tt = SSE(add)(SSE(load)(Y+2*ija_[kk]), SSE(mul)(xx, aa));
