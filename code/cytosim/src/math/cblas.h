...
inline double blas_dsdot(int N, const float* X, int incX, const float* Y, int incY)
{
    return dsdot_(&N, X, &incX, Y, &incY);
}

double sdsdot_(int*, const float* s, const float*, int*, const float*, int*);
inline double blas_sdsdot(int N, float SB, const float* X, int incX, const float* Y, int incY)
{
...
inline void blas_xscal(int N, real alpha, real*X, int incX)
{
    ...
    /**
     ...
     It is not a BLAS routine, but an addition of Intel Math Kernel Library
     */
    ...
    inline void blas_xaxpby(int N, real alpha, const real*X, int incX, real beta, real*Y, int incY)
    {
        if ( incX == 1  &&  incY == 1 )
        {
            for ( int ii = 0; ii < N; ++ii )
                Y[ii] = alpha * X[ii] + beta * Y[ii];
        }
        else
        {
            for ( int ii = 0; ii < N; ++ii )
                Y[ii*incY] = alpha * X[ii*incX] + beta * Y[ii*incY];
        }
