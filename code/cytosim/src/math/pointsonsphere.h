...
public:
    ...
    PointsOnSphere(Random&);
    
    /// constructor that also calls distributePoints(),
    PointsOnSphere(Random&, unsigned nbp);
    
    /// constructor that also calls distributePoints(), 
    PointsOnSphere(Random&, unsigned nbp, real precision, unsigned mx_nb_iterations);
    ...
    const real* addr(const unsigned ii) const { return &coord_[3 * ii]; }
    
    /// copy the coordinates from point ii onto the given 3-dim array x
    void   copyPoint(real x[3], unsigned ii);
    
    /// copy the coordinates from point ii onto x,y,z
    void   copyPoint(real* x, real* y, real* z, unsigned ii);
...
private:
    ...
    static const unsigned magic_seven = 7;
