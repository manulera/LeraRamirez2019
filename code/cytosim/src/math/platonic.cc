...
#pragma mark -
    ...
    void Solid::setCorner(unsigned inx, real x, real y, real z)
    {
        ...
        addVertex(Vertex(&corners_[inx], 1, 0, 0, 0, 0));
    }
...
#pragma mark -
    ...
    void Solid::refineEdge(unsigned a, unsigned b, unsigned div)
    {
        ...
        for ( unsigned ii = 1; ii < div; ++ii )
            addVertex(Vertex(A, div-ii, B, ii, 0, 0));
    }
    ...
    void Solid::refineFace(unsigned a, unsigned b, unsigned c, unsigned div)
    {
        ...
        for ( unsigned jj = 0;  jj <= div;  ++jj )
            line[jj] = getVertex(Vertex(A, div-jj, B, jj, 0, 0));
        
        for ( unsigned ii = 1;  ii <= div;  ++ii )
        {
            unsigned N = getVertex(Vertex(A, div-ii, C, ii, 0, 0));
            ...
            {
                unsigned N = getVertex(Vertex(A, div-ii-jj, B, jj, C, ii));
...
#pragma mark -
    ...
    {
#if ( 0 )
        const real x = (1+sqrt(5))/2;
        const real T = x/sqrt(1+x*x);
        const real Z = 1/sqrt(1+x*x);
#else
        const real T = 0.850650808352039932;
        const real Z = 0.525731112119133606;
#endif
    ...
    void Solid::setVertices(Polyhedra kind, unsigned div)
    {
        ...
        for ( unsigned n = 0; n < num_vertices_; ++n )
            vertices_[n].put(coordinates_+3*n);
    }
...
#pragma mark -
    ...
    void Vertex::put(float C[3]) const
    {
        C[0] = 0;
        C[1] = 0;
        C[2] = 0;
    ...
    void Vertex::put(double C[3]) const
    {
        C[0] = 0;
        C[1] = 0;
        C[2] = 0;
